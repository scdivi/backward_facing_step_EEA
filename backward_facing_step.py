...
# define empty arrays
ndofs = numpy.empty(nrefine)
qoi   = numpy.empty(nrefine)

# loop over number of refinement steps
with treelog.iter.fraction('level', range(nrefine)) as lrange:
    for irefine in lrange:
        ...
        ns.Q         = 'u_0 weight'
        qoi[irefine] = domain.integrate('Q dV' @ ns, degree=(degree+1)*2, arguments=dict(lhs=lhs))
        treelog.user(f'Quantity of interest: {qoi[irefine]}')
        
        # post-process
        util.plot_solution(domain, ns, lhs=lhs)
        util.plot_mesh(domain, geom)

# plot convergence
util.plot_convergence(ndofs, qoi)
